...
for im, seg_ims in tqdm(train_dict.iteritems(), total=len(train_dict.keys())):
	for seg_im in seg_ims:
		...
		bbs.append([im, seg_im, top, left, bottom, right])

df = pd.DataFrame().from_dict(bbs)
df.columns = ['image', 'mask', 'top', 'left', 'bottom', 'right']
df.to_csv('mask_bounding_boxes.csv', index=False)